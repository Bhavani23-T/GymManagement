...
if args["video_source"] is not None:
    ...
    if not cap.isOpened():
        print(f"Error: Could not open video file {args['video_source']}")
        exit(1)
else:
        ...
        if results.pose_landmarks is not None:
            try:
                ...
                if new_counter != counter:
                    print(f"{args['exercise_type'].capitalize()} count: {new_counter}")
                counter = new_counter
            except Exception as e:
                print(f"Detection error: {e}")
        else:
            print("Warning: No pose landmarks detected in this frame.")
